...
from PyQt5.QtWidgets import (
    QApplication, QDialog, QCheckBox, QGroupBox, QHBoxLayout,
    QVBoxLayout, QLabel, QLineEdit, QDialogButtonBox, QAction
)
...
class Login(QDialog):
    def __init__(self, parent=None, email=None, saved_password=None, remember_credentials=True):
        ...
        self.linkResetPassword = QLabel(
            ...
            + QApplication.translate("Plus", "Reset Password", None) + '</a></small>'
        )
        ...
        self.textName.setPlaceholderText(QApplication.translate("RoasterhubConfig", " Email", None))
        if email is not None:
            self.textName.setText(email)
        self.textName.textChanged.connect(self.textChanged)
        self.textPass = QLineEdit(self)
        self.textPass.setEchoMode(QLineEdit.Password)
        self.textPass.setPlaceholderText(QApplication.translate("RoasterhubConfig", "Password", None))
        
        self.textPass.textChanged.connect(self.textChanged)
                
        self.rememberCheckbox = QCheckBox(QApplication.translate("RoasterhubConfig", "Remember", None))
    ...
    def textChanged(self, _):
        ...
        if len(passwd) >= config.min_passwd_len \
                and len(login) >= config.min_login_len \
                and "@" in login and "." in login:
            self.dialogbuttons.button(QDialogButtonBox.Cancel).setDefault(False)
            self.dialogbuttons.button(QDialogButtonBox.Ok).setEnabled(True)
            self.dialogbuttons.button(QDialogButtonBox.Ok).setDefault(True)
        else:
...
def plus_login(window, email=None, saved_password=None, remember_credentials=True):
    ...
    l.setWindowTitle("Roasterhub ext")
